    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {scope === "custom" && (
            <div className="flex items-center gap-1.5 rounded-lg border border-sky-200 bg-sky-50 px-2.5 py-1 text-sm">
              <input
                type="number"
                min={1}
                max={records.length}
                ...
                className="w-16 rounded-md border border-slate-200 bg-white px-2 py-0.5 text-center font-semibold tabular-nums text-slate-800 focus:border-sky-400 focus:outline-none"
              />
              <span className="text-slate-500">
                days
                {String(customWindow) !== customDays.trim() && (
                  <span className="ml-1 text-sky-600">→ {customWindow}</span>
                )}
              </span>
            </div>
          )}
